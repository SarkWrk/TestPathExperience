...
local AIScript = coreScriptsFolder.AI
...
-- Sets up a table used for listening to AIScript events
listeners.AIScript = {}



-- Listens for AIScript:GetAttributeChangedSignal("InCycle"), and changes the main.IsIdled value to the inverse of it
function listeners.AIScript:InCycle() : nil
	if AIScript:GetAttribute("InCycle") == true then
		main.IsIdled = false
	else
		main.IsIdled = true
	end
...
-- Litens for AIScript:GetAttributeChangedSignal("Moving"), and changes the main.IsIdled value to it
function listeners.AIScript:Moving() : nil
	main.IsMoving = AIScript:GetAttribute("Moving")
end

-- Listens for AIScript:GetAttributeChangedSignal("OnMovingPlatform"), and if set to true stops WalkAnimation and RunAnimation
function listeners.AIScript:OnMovingPlatform() : nil
	if AIScript:GetAttribute("OnMovingPlatform") == true then
...
-- Connects the GetAttributeChangedSignal event of AIScripts's "InCycle" to listeners.AIScript:InCycle()
AIScript:GetAttributeChangedSignal("InCycle"):Connect(listeners.AIScript.InCycle)

-- Connects the GetAttributeChangedSignal event of AIScripts's "Moving" to listeners.AIScript:Moving()
AIScript:GetAttributeChangedSignal("Moving"):Connect(listeners.AIScript.Moving)
